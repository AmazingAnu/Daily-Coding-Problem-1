...
public class ClimbStaircase {
    ...
    public static void main(String[] args) {
        System.out.println(findNumberOfWays(4));

        int[] steps = {1,2};
        System.out.println(findNumberOfWaysForGivenSteps(4, steps));
    }
    ...
    private static int climbStairs(int[] memo,  int i, int n) {
        if (i > n) return 0;
        if (i == n) return 1;
    ...
    private static int findNumberOfWaysForGivenSteps(int n, int[] ways) {
        int[] memo = new int[n+1];
        return climbStairsInGivenWays(memo, 0, n, ways);
    }

    private static int climbStairsInGivenWays(int[] memo, int i, int n, int[] ways) {
        if (i > n) return 0;
        if (i == n) return 1;
